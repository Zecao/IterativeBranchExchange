...
% % remove o maior ciclo
% hashTSxArestasDoCiclo.remove(chaveMaiorCiclo);
...
% correcao de lacos por cluster
function novoInd = otimizaCicloClusterPvt2021(indTS,indBinOriginal,indTSOriginal,alim)
...
% de repetir alguma TS mais dfe 1 vez. 
...
for j=indices 
    ...
    if ( ~ismember(chaveMP,indTS) )  % if(chaveMP ~= indTS(j)) % OLD CODE
        ...
        % 2 FAZER FUNCAO QUE SUBSTITUI NOVO TS NO IND TS COMPLETO.
...
for TS=indTS;
    ...
    matrizTSxSegundoVerticeDaOrigem = [matrizTSxSegundoVerticeDaOrigem; TS, vertice1, vertice2];
end
